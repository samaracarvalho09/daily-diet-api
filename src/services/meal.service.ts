...
export const createMeal = async (data: { name: string, description?: string, date: Date, isOnDiet: boolean, userId: string }) => {
...
export const getMealById = async (id: string, userId: string) => {
  ...
  if (!meal) throw new Error("Refeição não encontrada ou não pertence ao usuário");
  return meal;
};

export const updateMeal = async (id: string, userId: string, data: Partial<any>) => {
  const meal = await prisma.meal.findFirst({ where: { id, userId } });
...
// Calculates the longest consecutive on-diet streak
  ...
  for (const meal of meals) {
    if (meal.isOnDiet) {
      currentSequence += 1;
      if (currentSequence > bestOnDietSequence) bestOnDietSequence = currentSequence;
    } else {
      currentSequence = 0;
    }
  }
